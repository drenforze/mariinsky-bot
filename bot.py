...
async def check_all(bot: Bot):
    ...
    async with async_playwright() as pw:
        ...
        historical_events = [
            e for e in events
            if "мариинский театр" in e.get("venue", "").lower()
            and "мариинский-2" not in e.get("venue", "").lower()
            and "концертный" not in e.get("venue", "").lower()
        ]
        logger.info(f"Спектаклей на исторической сцене: {len(historical_events)}")
