...
async def check_restricted_tickets(page, event: dict) -> list:
    ...
    Возвращает список найденных мест: [{info, price}, ...]
    """
    ...
    found_seats = []
    try:
        # domcontentloaded вместо networkidle — в 3–4 раза быстрее
        await page.goto(event["link"], wait_until="domcontentloaded", timeout=15000)
        # Ждём только появления нужного текста, не всей страницы
        try:
...
async def check_all(bot: Bot):
    ...
    async with async_playwright() as pw:
        ...
        # Только историческая сцена — «Мариинский театр» (не Мариинский-2, не Концертный зал)
        # Только ближайшие 7 дней
        today = datetime.now().date()
        deadline = today + timedelta(days=7)

        def parse_event_date(date_str: str):
            """Пытается распознать дату из строки вида '7 марта', '07.03.2026' и т.п."""
            months = {
            ...
            try:
                # Формат: "7 марта" или "7 марта 2026"
        ...
        for event in historical_events:
            ...
            await asyncio.sleep(0.3)  # небольшая пауза между спектаклями
...
async def cmd_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Проверяю афишу и билеты (~20–30 сек)...")
